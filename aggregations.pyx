...
def recursive_sma(
    const float64_t[:] values, int window
) -> np.ndaray:
    return _recursive_sma(values, window)


cdef float64_t[:] _recursive_sma(const float64_t[:] values, int window):
    cdef:
        float64_t[:] output
        Py_ssize_t i, in_n
    ...
    for i in range(window - 1):
        output[i] = NaN
    output[window - 1] = np.average(np.asarray(values[:window]))
    for i in range(in_n - window):
        output[i + window] = output[i + window - 1] - values[i] / window + values[i + window] / window
    return output
